...
class CameraAugmentorV11:
    ...
    tz_post_min = 0.5
    # FoV safety margin: 把"虚拟画框"按比例缩小再做超界判定, 等价于真画框
    # 留 (1-margin) 的 padding. margin=0.85 表示画面四周各留约 7.5% 余量.
    fov_safety_margin = 0.85

    def __init__(self):
        self.w = 1000
        # Real K used by training (base_dataset) is from create_camera_sensor(1000, 1000, 43.3),
        # which gives f ≈ 1415 and half-fov ≈ 0.353 rad (~20°). Previously this class used 24mm
        # (f≈784, half-fov ≈0.64) which is much wider than the real training camera, so the
        # push-away logic let subjects sit outside the actual training FoV. Match the real K.
        self.f = create_camera_sensor(1000, 1000, 43.3)[2][0, 0]
    ...
    def __call__(self, w_j3d, length=120):
        ...
        if c_j3d_min < self.tz_post_min:
            ...
            delta[..., 2] += push_away
            c_j3d[..., 2] += push_away
        #   - If ANY joint is not in the FoV (with a safety margin), push away in z
        #   Use all joints (not just root) so hands / feet / head also stay in frame.
        #   safety_margin < 1.0 leaves padding around the frame border ("放大画框").
        safety_margin = self.fov_safety_margin
        c_j_safe_z = c_j3d[..., 2:3].clamp(min=self.tz_post_min)
        half_fov_all = torch.div(c_j3d[..., :2], c_j_safe_z).abs()  # (L, J, 2) ratio
        max_fov = half_fov_all.max()
        target_tol = self.half_fov_tol * safety_margin
        if max_fov > target_tol:
            # find the worst-offending (frame, joint, axis) and push far enough that ratio ≤ target_tol
            flat_idx = torch.argmax(half_fov_all.flatten())
            l_idx = flat_idx // (half_fov_all.shape[1] * 2)
            jr     = flat_idx %  (half_fov_all.shape[1] * 2)
            j_idx  = jr // 2
            a_idx  = jr %  2
            xy = c_j3d[l_idx, j_idx, a_idx].abs()
            z_trg = xy / target_tol  # depth needed so ratio == target_tol
            # clamp(min=0): only push the camera *away*, never forward. Without
            # this, a far-frame's z can already exceed z_trg → push_away < 0 →
            # camera gets pulled behind the subject and tz_post_min protection
            # (above) is silently broken.
            push_away = (z_trg - c_j3d[l_idx, j_idx, 2]).clamp(min=0)
            delta[..., 2] += push_away
        t_w2c += delta
...
class StaticCameraV11:
    """全程静止单一相机, 与训练 CameraAugmentorV11 "static" 类型分布同源 (训练 40% 概率).

    设计动机 (长序列推理):
      - 整段一次性喂网络时, "random/track" 类相机的 push-away/FoV 检查针对几千帧最远那一刻
        触发, 相机被推得远远超过训练 120 帧窗口下的距离, bbx_xys 整体缩水 → obs 数值落到
        训练分布尾巴.
      - 静止相机的 cam_angvel ≡ 0, 也是训练里常见情形 (static 路径不调 add_stepnoise/
        rotation move), 在推理时少一个 OOD 维度.
      - K_fullimg / 复用 create_camera 的 yaw/pitch/roll/tx/ty/tz 分布与训练一致, 仅保留
        push-away/FoV postproc 让长序列里 subject 不会跑出画面.

    与训练分布的唯一差异: FoV 检查在整段 L 帧上做最大值 (训练只看 120 帧), 长 motion 时
    相机会比单窗静态平均值略远. 这是 "全长 forward" 一次性方案不可避免的代价, 但比 random/
    track 类多旋转噪声引发的 cam_angvel 漂移小得多.

    可复现性: 每次 __call__ 内部 reseed np.random (用 seed + key_hash), 同一条 motion 跨
    run 拿到一模一样的相机. seed 在外部由 dataset 决定 (默认 0).
    """

    def __init__(self, seed: int = 0):
        # 与 CameraAugmentorV11.__init__ 完全一致, 保证 K/FoV 阈值与训练同源.
        self.w = 1000
        self.f = create_camera_sensor(1000, 1000, 43.3)[2][0, 0]
        self.half_fov_tol = (self.w / 2) / self.f
        self.tz_post_min = 0.5
        self.fov_safety_margin = 0.85
        self.cfg_create_camera = CameraAugmentorV11.cfg_create_camera
        self.seed = int(seed)

    def __call__(self, w_j3d, length=None):
        """
        Args:
            w_j3d : (L, J, 3) 全段世界系关节点, y-up.
            length: 可选, 默认 = w_j3d.shape[0].
        Returns:
            T_w2c : (L, 4, 4)
        """
        L = int(length) if length is not None else int(w_j3d.shape[0])
        assert w_j3d.size(0) == L, f"w_j3d.shape[0]={w_j3d.shape[0]} != length={L}"

        w_j3d = w_j3d.clone()
        w_root = w_j3d[:, 0]                                            # (L, 3) pelvis

        # ---- (1) 用 create_camera 抽一台静止相机 (yaw/pitch/roll/tx/ty/tz 随机分布与训练一致) ----
        np_state = np.random.get_state()
        torch_state = torch.random.get_rng_state()
        np.random.seed(self.seed)
        torch.manual_seed(self.seed)
        try:
            cfg = {**self.cfg_create_camera, "w": self.w, "f": self.f}
            R0_w2c, t0_w2c = create_camera(w_root[0], cfg)              # (3,3) (3,)
        finally:
            np.random.set_state(np_state)
            torch.random.set_rng_state(torch_state)

        R_w2c = R0_w2c.repeat(L, 1, 1)                                  # (L, 3, 3)
        t_w2c = t0_w2c.repeat(L, 1)                                     # (L, 3)

        # ---- (2) push-away + FoV postproc (与 CameraAugmentorV11.__call__ 末尾同款) ----
        # 让 subject 全程留在画面里. 长 motion 这一步会把相机推得比单窗静态相机更远.
        c_j3d = torch.einsum("lij,lkj->lki", R_w2c, w_j3d) + t_w2c[:, None]  # (L, J, 3)
        delta = torch.zeros_like(t_w2c)

        c_j3d_min = c_j3d[..., 2].min()
        if c_j3d_min < self.tz_post_min:
            push_away = self.tz_post_min - c_j3d_min
            delta[..., 2] += push_away
            c_j3d[..., 2] += push_away
        ...
        c_j_safe_z = c_j3d[..., 2:3].clamp(min=self.tz_post_min)
        half_fov_all = torch.div(c_j3d[..., :2], c_j_safe_z).abs()       # (L, J, 2)
        max_fov = half_fov_all.max()
        target_tol = self.half_fov_tol * self.fov_safety_margin
        if max_fov > target_tol:
            flat_idx = torch.argmax(half_fov_all.flatten())
            l_idx = flat_idx // (half_fov_all.shape[1] * 2)
            jr     = flat_idx %  (half_fov_all.shape[1] * 2)
        ...
        T_w2c = transform_mat(R_w2c, t_w2c)                              # (L, 4, 4)
        return T_w2c
