...
def compute_bbox_info_bedlam(bbx_xys, K_fullimg):
    ...
    bbox_info = bbox_info / fl.clamp(min=1e-6)
    return bbox_info
...
def get_a_pred_cam(transl, bbx_xys, K_fullimg):
    ...
    cx = K_fullimg[..., 0, 2]
    cy = K_fullimg[..., 1, 2]
    f_safe = f.clamp(min=1e-6)
    denom = (transl[..., 2] * bbx_xys[..., 2]).clamp(min=1e-6)
    gt_s = 2 * f / denom  # (B, L)
    gt_x = transl[..., 0] - transl[..., 2] / f_safe * (bbx_xys[..., 0] - cx)
    gt_y = transl[..., 1] - transl[..., 2] / f_safe * (bbx_xys[..., 1] - cy)
...
def normalize_kp2d(obs_kp2d, bbx_xys, clamp_scale_min=False):
    ...
    # Always clamp scale to avoid div-by-zero on degenerate bboxes
    # (clamp_scale_min kept for compat — already covered).
